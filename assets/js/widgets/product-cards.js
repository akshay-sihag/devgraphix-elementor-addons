/* =========================================================================
 * Devgraphix — Product Cards carousel
 * Lightweight, dependency-free scroll-snap carousel: prev/next arrows,
 * generated dots, optional autoplay + loop, pause on interaction. Initialises
 * itself on the frontend and inside the Elementor editor (MutationObserver).
 * ========================================================================= */
( function () {
	'use strict';
	...
	function buildCarousel( root ) {
		if ( root.dataset.dgxPcInit === '1' ) {
			return;
		}
		root.dataset.dgxPcInit = '1';

		var viewport = root.querySelector( '.dgx-pcards__viewport' );
		var track = root.querySelector( '.dgx-pcards__track' );
		if ( ! viewport || ! track ) {
			return;
		}
		...
		var prevBtn = root.querySelector( '.dgx-pcards__arrow--prev' );
		var nextBtn = root.querySelector( '.dgx-pcards__arrow--next' );
		var dotsWrap = root.querySelector( '.dgx-pcards__dots' );

		var autoplay = root.dataset.autoplay === 'yes';
		var interval = num( root.dataset.interval, 5000 );
	...
	function initAll( ctx ) {
		...
		var nodes = scope.querySelectorAll( '.dgx-pcards--carousel' );
		Array.prototype.forEach.call( nodes, buildCarousel );
	}
